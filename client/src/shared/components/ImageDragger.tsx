...
export const ImageDragger = ({
  ...
  return (
    <Dragger {...uploadProps}>
      <Flex justify="center">
        {previewUrl ? (
          <img src={previewUrl} height={60} alt="Preview" style={{ objectFit: 'contain' }} />
        ) : (
          <img src={filesImage} height={30} alt="Upload" />
        )}
      </Flex>
      <Flex vertical>
        <Text>{previewUrl ? 'Click or drag file to replace' : hintText}</Text>
        <Text type="secondary">
          Support for image files (JPG, PNG, GIF, WEBP, BMP, SVG). Maximum size: 5MB
        </Text>
      </Flex>
    </Dragger>
  )
